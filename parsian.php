...
class Parsian 
{
	...
	var $callback_function = 'index';
	var $callback_url;
	var $callback_method = "CI";
	var $payment_model;
	var $payment_function;	
	var $CI;
	


	/*
	* $config = array('pin' => "pincode from parsian bank", 'callback_controller' , 'callback_function', 'callback_url', 'callback_method')
	*/
    ...
    public function do_payment($paymentID, $amount, $config)
    {

	if(isset($config['callback_method']))
	{		
		if($config['callback_method'] == 'CI')
		{
			if(isset($config['callback_controller']))
				$this->callback_controller = $config['callback_controller'];
			else
				return 2;
			if(isset($config['callback_function']))
				$this->callback_function = $config['callback_function'];
		}
		else if($config['callback_method'] == 'url')
		{
			$this->callback_method = 'url';
		...
		return 2;
	if(isset($config['payment_model']))
		$this->payment_model = $config['payment_model'];
	else
		return 2;
	if(isset($config['payment_function']))
		$this->payment_function = $config['payment_function'];
	else
		return 2;
    	...
    	{
    		$r['done']= false;
    		return $r;//خطای اتصال به بانک
    	}
    	$pin = $this->pin;
	if($this->callback_method == 'CI')
    		$callback = site_url($this->callback_controller.'/'.$this->callback_function.'/'.$paymentID);//http://example.com/index.php/controller/function/12342
	else if($this->callback_method == 'url')
		$callback = $this->callback_url."?oid=$paymentID";//http://example.com/some_file.php?pid=12342
    	$price = $amount;
    	$params = array(
    			'pin' => $pin ,
    			'amount' => $price ,
   	...
   	$error_num = array();
		...
		$this->save_payment($data);
    		redirect("https://www.pec24.com/pecpaymentgateway/?au=".$res['authority'] , "refresh");
                return true;
    	}
    }//End of function do_payment
    
    public function settle($paymentID, $au)
    {
    	$soapclient = new nusoap_client('https://www.pec24.com/pecpaymentgateway/eshopservice.asmx?wsdl','wsdl');    	
    	$err = $soapclient->getError();   	
    	if (!$err)
	...
	$func = $this->payment_func;
	$this->CI->load->model($model);
	$this->CI->$model->$func($data);
     }//End of function save_payment
}

/* End of file parsian.php */
